...
namespace BuildingTools
{
    public class TextInputWithKeyListener<T> : TextInput<T>
    {
        ...
        public static TextInputWithKeyListener<T> Quick(T subject, IVS<string, T> getString, string label, ToolTip tip,
            Action<T, string> changeAction, params KeyPressEvent[] events) => new TextInputWithKeyListener<T>(subject, getString, M.m<T>(label), M.m<T>(tip), changeAction, events, null, s => s, (x, s) => null);
